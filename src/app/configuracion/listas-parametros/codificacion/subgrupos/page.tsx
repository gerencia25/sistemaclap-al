...
export default function SubgruposCodificacionPage() {
  const { session } = useAuth();

  const [items, setItems] = useState<ItemSubgroup[]>([]);
  ...
  const apiUrl =
    "/api/configuracion/listas-parametros/codificacion/subgrupos";
  ...
  const loadItems = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          result.error ||
            "No se pudieron cargar los subgrupos de codificación.",
        );
      }
  ...
  function openEditForm(item: ItemSubgroup) {
    setForm({
      id: item.id,
  ...
  async function toggleStatus(item: ItemSubgroup) {
    if (!session?.access_token) return;
    ...
    try {
      ...
      setSuccessMessage(
        nextStatus === "Activo"
          ? "Subgrupo activado correctamente."
          : "Subgrupo inactivado correctamente.",
      );
    ...
    <div className="space-y-8">
      <section className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
        <div className="max-w-4xl">
          ...
          <p className="mt-3 max-w-3xl text-base leading-7 text-gray-600">
            Administra los subgrupos pertenecientes a cada grupo y categoría
            utilizados para construir la clasificación de los productos.
          </p>
        </div>
      ...
      <section className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
        <div className="border-b border-gray-100 px-5 py-4">
          <h2 className="font-semibold text-gray-900">
            Subgrupos configurados
          </h2>

          <p className="mt-1 text-sm text-gray-500">
            {items.length} subgrupo
            {items.length === 1 ? "" : "s"}
          </p>
        </div>
      ...
      <section className="rounded-2xl border border-blue-100 bg-blue-50/50 p-5">
        ...
        <p className="mt-2 max-w-4xl text-sm leading-6 text-gray-600">
          Cuando un subgrupo ya tenga una plantilla de codificación o
          productos relacionados, su grupo padre y su código técnico quedarán
                  ...
                  {groups
                    ...
                    .map((group) => (
                      <option
                        ...
                        value={group.id}
                      >
                        {group.category.code} · {group.code} · {group.name}
                        {group.status === "Inactivo" ||
                        group.category.status === "Inactivo"
                          ? " · Inactivo"
                          : ""}
                      </option>
                    ))}
              ...
              {form.id && form.structure_locked && (
                <div className="rounded-2xl border border-amber-100 bg-amber-50 p-4">
                  ...
                  <p className="mt-1 text-sm leading-6 text-amber-700">
                    Este subgrupo ya tiene información relacionada. Su grupo
                    padre y código técnico no pueden modificarse.
                  </p>
