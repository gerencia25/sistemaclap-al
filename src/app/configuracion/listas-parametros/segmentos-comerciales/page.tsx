...
export default function SegmentosComercialesPage() {
  const { session } = useAuth();

  const [items, setItems] = useState<CustomerSegment[]>([]);
  ...
  const apiUrl =
    "/api/configuracion/listas-parametros/segmentos-comerciales";
  ...
  const loadItems = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          result.error ||
            "No se pudieron cargar los segmentos comerciales.",
        );
      }
  ...
  function openEditForm(item: CustomerSegment) {
    setForm({
      id: item.id,
      code: item.code,
      name: item.name,
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          result.error ||
            "No se pudo guardar el segmento comercial.",
        );
      }

      setSuccessMessage(
        isEditing
          ? "Segmento comercial actualizado correctamente."
          : "Segmento comercial creado correctamente.",
      );
  ...
  async function toggleStatus(item: CustomerSegment) {
    if (!session?.access_token) return;
    ...
    try {
      ...
      setSuccessMessage(
        nextStatus === "Activo"
          ? "Segmento comercial activado correctamente."
          : "Segmento comercial inactivado correctamente.",
      );
    ...
    <div className="space-y-8">
      <section className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
        <div className="max-w-4xl">
          ...
          <p className="mt-3 max-w-3xl text-base leading-7 text-gray-600">
            Administra las clasificaciones comerciales utilizadas para
            clientes, prospectos, listas de precios y cotizaciones.
          </p>
        </div>
      ...
      <section className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
        ...
        {isLoading ? (
          <div className="px-6 py-12 text-center text-sm text-gray-500">
            Cargando segmentos comerciales...
          </div>
        ) : items.length === 0 ? (
          <div className="px-6 py-14 text-center">
            <p className="font-semibold text-gray-900">
              No hay segmentos comerciales configurados.
            </p>
      ...
      <section className="rounded-2xl border border-blue-100 bg-blue-50/50 p-5">
        <p className="text-sm font-semibold text-[#07076b]">
          Conservamos la trazabilidad
        </p>

        <p className="mt-2 text-sm leading-6 text-gray-600">
          Los segmentos que dejen de utilizarse se marcan como inactivos.
          Permanecen relacionados con clientes y cotizaciones históricas, pero
          dejan de aparecer para nuevos registros.
        </p>
      </section>
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <h2 className="mt-2 text-2xl font-bold text-[#07076b]">
                  {form.id
                    ? "Editar segmento comercial"
                    : "Nuevo segmento comercial"}
                </h2>
              </div>
            ...
            >
              ...
              <Field label="Descripción">
                <textarea
                  ...
                  rows={3}
                  className={inputClassName}
                  placeholder="Descripción interna del segmento comercial."
                />
              </Field>
