...
type ThirdPartyRequestAttachment = {
  created_third_party_id: string;
  rut_file_url: string | null;
  chamber_commerce_file_url: string | null;
  customer_knowledge_file_url: string | null;
  supplier_knowledge_file_url: string | null;
...
const emptyThirdParty: Omit<
  ThirdParty,
  "id" | "siigo_third_party_id" | "siigo_last_sync_at"
> = {
  is_customer: true,
  is_supplier: false,
  is_other: false,
...
function TercerosContent() {
  ...
  const [linkedRequest, setLinkedRequest] = useState<ThirdPartyRequest | null>(
    null,
  );

  const [requestAttachments, setRequestAttachments] = useState<
    ThirdPartyRequestAttachment[]
  >([]);
  ...
  const [identificationTypes, setIdentificationTypes] = useState<
    MasterOption[]
  >([]);
  const [fiscalResponsibilities, setFiscalResponsibilities] = useState<
    MasterOption[]
  >([]);
...
const isEditing = editingThirdPartyId !== null;
  ...
  setLinkedRequest(data as ThirdPartyRequest);

  setNewThirdParty((current) => ({
    ...current,
    is_customer: data.category === "Cliente",
    is_supplier: data.category === "Proveedor",
    is_other: false,
  }));

  setIsModalOpen(true);
}
  async function fetchRequestAttachments() {
    const { data, error } = await supabase
      .from("third_party_requests")
      .select(
        "created_third_party_id, rut_file_url, rut_file_name, chamber_commerce_file_url, chamber_commerce_file_name, customer_knowledge_file_url, customer_knowledge_file_name, supplier_knowledge_file_url, supplier_knowledge_file_name",
      )
      .not("created_third_party_id", "is", null);

    if (error) {
      alert(`Error cargando adjuntos: ${error.message}`);
      return;
    }

    setRequestAttachments((data ?? []) as ThirdPartyRequestAttachment[]);
  }
  ...
  async function fetchMasterData() {
    ...
    setIdentificationTypes(
      (identificationTypesResponse.data ?? []) as MasterOption[],
    );
    setFiscalResponsibilities(
      (fiscalResponsibilitiesResponse.data ?? []) as MasterOption[],
    );
  ...
  function getAttachmentsByThirdPartyId(thirdPartyId: string) {
    return requestAttachments.find(
      (attachment) => attachment.created_third_party_id === thirdPartyId,
    );
  }
  ...
  function handleEditThirdParty(thirdParty: ThirdParty) {
  setEditingThirdPartyId(thirdParty.id);

  setNewThirdParty({
    is_customer: thirdParty.is_customer,
    is_supplier: thirdParty.is_supplier,
    is_other: thirdParty.is_other,
    ...
    verification_digit: thirdParty.verification_digit ?? "",
    first_name: thirdParty.first_name ?? "",
    last_name: thirdParty.last_name ?? "",
    business_name: thirdParty.business_name ?? "",
    commercial_name: thirdParty.commercial_name ?? "",

    country: thirdParty.country ?? "Colombia",
    department: thirdParty.department ?? "",
    city: thirdParty.city ?? "",
    address: thirdParty.address ?? "",
    phone_indicative: thirdParty.phone_indicative ?? "57",
    phone: thirdParty.phone ?? "",
    email: thirdParty.email ?? "",

    electronic_invoice_email: thirdParty.electronic_invoice_email ?? "",
    fiscal_responsibility:
      thirdParty.fiscal_responsibility ?? "R-99-PN - No aplica / Otros",
    iva_regime: thirdParty.iva_regime ?? "No aplica",
    ciiu_activity: thirdParty.ciiu_activity ?? "",
    postal_code: thirdParty.postal_code ?? "",
    is_vat_responsible: thirdParty.is_vat_responsible,
    is_grand_taxpayer: thirdParty.is_grand_taxpayer,
    is_self_withholder: thirdParty.is_self_withholder,
    is_vat_withholding_agent: thirdParty.is_vat_withholding_agent,

    payment_terms: thirdParty.payment_terms ?? "",
    currency: thirdParty.currency ?? "COP",
    credit_limit: thirdParty.credit_limit ?? 0,
    price_list: thirdParty.price_list ?? "",
    seller: thirdParty.seller ?? "",
    collector: thirdParty.collector ?? "",

    siigo_sync_enabled: thirdParty.siigo_sync_enabled,
    siigo_sync_status: thirdParty.siigo_sync_status ?? "Pendiente",
  ...
  async function handleSaveThirdParty() {
    ...
    try {
      const payload = {
        ...
        fiscal_responsibility: newThirdParty.fiscal_responsibility || null,
      ...
      let savedThirdPartyId = editingThirdPartyId;

if (isEditing) {
  const { error } = await supabase
    .from("third_parties")
    .update(payload)
    .eq("id", editingThirdPartyId);
  ...
  if (error) {
    if (
      error.message.includes(
        "third_parties_identification_type_identification_number_key",
      )
    ) {
      throw new Error(
        "Ya existe un tercero registrado con ese tipo y número de identificación.",
      );
    }
  ...
  savedThirdPartyId = insertedThirdParty.id;
}
  ...
  const { error: requestUpdateError } = await supabase
    ...
    .update({
  status: "Creado",
  created_third_party_id: savedThirdPartyId,
            ...
            <tbody className="divide-y divide-gray-200">
              ...
              {!isLoading &&
                filteredThirdParties.map((thirdParty) => {
                  const attachments = getAttachmentsByThirdPartyId(
                    thirdParty.id,
                  );
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-4">
                        {attachments ? (
                          <div className="flex flex-wrap gap-2">
                            {attachments.rut_file_url && (
                              <AttachmentButton
                                label="RUT"
                                url={attachments.rut_file_url}
                              />
                            )}

                            {attachments.chamber_commerce_file_url && (
                              <AttachmentButton
                                label="Cámara"
                                url={attachments.chamber_commerce_file_url}
                              />
                            )}

                            {attachments.customer_knowledge_file_url && (
                              <AttachmentButton
                                label="Cliente"
                                url={attachments.customer_knowledge_file_url}
                              />
                            )}

                            {attachments.supplier_knowledge_file_url && (
                              <AttachmentButton
                                label="Proveedor"
                                url={attachments.supplier_knowledge_file_url}
                              />
                            )}
                          </div>
                        ) : (
...
<h2 className="text-2xl font-bold text-[#07076b]">
  ...
  ? "Editar tercero"
  : linkedRequest
    ? `Crear tercero desde solicitud ${linkedRequest.request_number}`
    : "Nuevo tercero"}
</h2>
  ...
  <section className="mb-8 rounded-2xl border border-indigo-100 bg-indigo-50 p-5">
    <div className="mb-4 flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
      <div>
        ...
        <h3 className="mt-1 text-xl font-bold text-[#07076b]">
    ...
    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
      <DetailMini label="Solicitante" value={linkedRequest.requester_name} />
      <DetailMini label="Área" value={linkedRequest.requester_area} />
      <DetailMini label="Cargo" value={linkedRequest.requester_position} />
      <DetailMini label="Categoría" value={linkedRequest.category} />
      <DetailMini label="Tipo solicitud" value={linkedRequest.request_type} />
      <DetailMini label="Correo" value={linkedRequest.requester_email} />
    </div>
  ...
  <div className="flex flex-wrap gap-2">
    {linkedRequest.rut_file_url && (
      <AttachmentButton label="Ver RUT" url={linkedRequest.rut_file_url} />
    )}

    {linkedRequest.chamber_commerce_file_url && (
      <AttachmentButton
        label="Ver Cámara de comercio"
        url={linkedRequest.chamber_commerce_file_url}
      />
    )}

    {linkedRequest.customer_knowledge_file_url && (
      <AttachmentButton
        label="Ver Conocimiento cliente"
        url={linkedRequest.customer_knowledge_file_url}
      />
    )}

    {linkedRequest.supplier_knowledge_file_url && (
      <AttachmentButton
        label="Ver Conocimiento proveedor"
        url={linkedRequest.supplier_knowledge_file_url}
      />
    )}
  </div>
</div>
            ...
            <section className="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm">
              <div className="mb-6 border-b border-gray-100 pb-5">
                ...
                <p className="mt-1 text-sm text-gray-500">
                  Un tercero puede ser cliente, proveedor u otro al mismo
                  tiempo.
                </p>
              </div>
              ...
              <div className="grid gap-4 md:grid-cols-3">
                <CheckboxField
                  ...
                  onChange={(value) =>
                    updateNewThirdParty("is_customer", value)
                  }
                />
                <CheckboxField
                  label="Proveedor"
                  ...
                  onChange={(value) =>
                    updateNewThirdParty("is_supplier", value)
                  }
                />
                <CheckboxField
                  label="Otro"
            ...
            <section className="mt-8 rounded-3xl border border-indigo-100 bg-indigo-50 p-6 shadow-sm">
              <div className="mb-6 border-b border-indigo-100 pb-5">
                ...
                <p className="mt-1 text-sm text-gray-600">
                  Información clave para facturación electrónica,
                  responsabilidades fiscales y datos tributarios.
                </p>
              </div>
                ...
                <Field label="Responsabilidad fiscal">
                  ...
                  >
                    ...
                    {fiscalResponsibilities.map((responsibility) => {
                      const label =
                        getFiscalResponsibilityLabel(responsibility);
                      return (
                  ...
                  <div className="mt-3 rounded-2xl border border-gray-200 bg-gray-50 p-5">
                    <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                      <div>
                        ...
                        <p className="mt-1 text-sm text-gray-500">
                          Agrega las personas de contacto para comunicación
                          comercial, compras, cartera o facturación.
                        </p>
                      </div>
            ...
            <section className="mt-8 rounded-3xl border border-indigo-100 bg-indigo-50 p-6 shadow-sm">
              <div className="mb-6 border-b border-indigo-100 pb-5">
                ...
                <p className="mt-1 text-sm text-gray-600">
                  Controla si este tercero debe sincronizarse con SIIGO y
                  consulta su estado de integración.
                </p>
              </div>
...
export default function TercerosPage() {
  return (
    <Suspense fallback={<div className="text-sm text-gray-500">Cargando terceros...</div>}>
      <TercerosContent />
    </Suspense>
  );
}
...
function AttachmentButton({ label, url }: { label: string; url: string }) {
  return (
    <a
      href={url}
      target="_blank"
