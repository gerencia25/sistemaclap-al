...
export default function SolicitudTercerosPage() {
  ...
  const [isLoadingRequester, setIsLoadingRequester] = useState(true);
...
function resetForm() {
  setForm((current) => ({
    ...
    requester_position: current.requester_position,
    ...
    <div className="space-y-8">
      <section>
        ...
        <p className="mt-3 max-w-4xl text-base leading-7 text-gray-600">
          Formulario oficial de Dirección Comercial para solicitar a Dirección Financiera la creación o desactivación de clientes y proveedores.
        </p>
      </section>
  ...
  <Field label="Cargo">
    <input
      value={
        isLoadingRequester
          ? "Cargando..."
          : form.requester_position || "Sin cargo registrado"
      }
      readOnly
      className={`${inputClassName} cursor-not-allowed bg-gray-50 text-gray-600`}
    />
